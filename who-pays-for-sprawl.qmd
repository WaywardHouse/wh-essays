...
(function(){
  function tryInit() {
    ...
    [...calgaryZones, ...edmontonZones].forEach(z => {
      L.polygon([z.coords[0].map(([lat,lng]) => [lat,lng])], {color:'white',weight:1.5,fillColor:z.color,fillOpacity:0.25})
...
The City of Edmonton's 2022 infrastructure report estimated a current infrastructure renewal gap of approximately $6.4 billion, growing by roughly $250 million annually [@city-of-edmonton-infrastructure2022]. The City of Calgary's equivalent figure is likely comparable or larger, reflecting both the scale of the city's suburban build-out and the age of inner-city infrastructure. Smaller cities that grew rapidly in the resource boom years — Fort McMurray, Grande Prairie, Lloydminster — face proportionally even larger gaps, because their growth was faster and their tax bases are more exposed to energy sector cyclicality.
...
*Related reading: [Hidden in Plain Sight](/articles/hidden-in-plain-sight.html) examines the greenspace infrastructure underlying Calgary's livability; [The Distance to Care](/articles/the-distance-to-care.html) traces how settlement patterns shape healthcare access in the province.*
